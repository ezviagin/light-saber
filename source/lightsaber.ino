...
// 0 - red, 1 - blue, 2 - green, 3 - bink, 4 - yellow
byte LEDcolor;  
...
void setup() 
{
	...
	FastLED.setBrightness(100);  // led brightness == 40%
	setAll(0, 0, 0);
	...
	randomSeed(analogRead(2)); // reference point for random numbers
	...
	if ((EEPROM.read(0) >= 0) && (EEPROM.read(0) <= 5)) {  // if firmware was loaded 1 or more times
		nowColor = EEPROM.read(0); // set last color
		HUMmode = EEPROM.read(1); // set last mode
	}
	else {
		EEPROM.write(0, 0);
		EEPROM.write(1, 0);
	...
	for (char i = 0; i <= capacity; i++) { // charge as light lenght
...
void btnTick() 
{
	btnState = !digitalRead(BTN); // if pressed
	...
	if (!btnState && btn_flag) { // if pressed and released
		btn_flag = 0;
		hold_flag = 0; // reset hold flag
	}

	// button is pressed
	if (btn_flag && btnState && (millis() - btn_timer > BTN_TIMEOUT) && !hold_flag) {
		ls_chg_state = 1; // turn on / off
		hold_flag = 1;
		btn_counter = 0;
	}
  
	// button is pressed several times before timeout
...
void getFreq() 
{
	if (!ls_state) // if sabed is turned off
		return;
	...
	if (millis() - mpuTimer > 500) {                            // каждые полмиллисекунды
		...
		mpuTimer = micros(); // reset times
	}
}

// choose one diod color
...
// smooth sword off
void light_down() {
	for (char i = (NUM_LEDS / 2 - 1); i >= 0; i--) { // leds off
		setPixel(i, 0, 0, 0);
...
void setColor(byte color) {
	// 0 - red, 1 - blue, 2 - green, 3 - pink, 4 - yellow, 5 - light blue
	switch (color) 
	{
	case 0:
		red = 255;
